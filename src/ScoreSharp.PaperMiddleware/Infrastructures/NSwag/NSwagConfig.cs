...
namespace ScoreSharp.PaperMiddleware.Infrastructures.Nswag;
...
public static class NSwagConfig
{
    public static void AddNSwag(this IServiceCollection services, IHostEnvironment env)
    {
        ...
        services.AddOpenApiDocument(
            (settings, provider) =>
            {
                settings.Title = $"信用卡徵審系統 API PaperMiddleware ({env.EnvironmentName})";
                settings.Version = "v1";
                settings.Description = @"ScoreSharp PaperMiddleware API";
                ...
                settings.PostProcess = (document) =>
                {
                    ...
                    if (httpContext != null)
                    {
                        ...
                        document.Servers.Add(
                            new OpenApiServer { Url = $"{scheme}://{host}", Description = $"當前環境: {env.EnvironmentName}" }
                        );
                    }
                };
            }
