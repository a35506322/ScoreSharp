...
public class SyncApplyInfoPaperRequest : IValidatableObject
{
    ...
    /// <summary>
    /// 本人同意提供資料予聯名(認同)集團(認同)集團
    /// 定義值：Y/N
    /// </summary>
    [Display(Name = "本人同意提供資料予聯名(認同)集團(認同)集團")]
    [MaxLength(1)]
    [RegularExpression("^[YN]$")]
...
public static class 參數類別
{
    ...
    public static readonly string 專案代號 = "ProjectCode";
    public static readonly string 縣市 = "City";
    public static readonly string 年費收取方式 = "AnnualFeeCollectionMethod";
}

public class FieldValidationCodes
{
    public string AMLProfessionOtherCode { get; set; }
    public string MainIncomeAndFundOtherCode { get; set; }
}
