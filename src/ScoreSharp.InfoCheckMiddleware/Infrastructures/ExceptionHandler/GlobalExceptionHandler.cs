    ...
    : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext httpContext, Exception exception, CancellationToken cancellationToken)
    {
        ...
        if (exception is BadRequestException badRequestEx)
        {
            ...
            errorResponse = ApiResponseHelper.BadRequest(errors: badRequestEx.ValidationErrors);
        }
        else if (exception is NotFoundException notFoundEx)
        {
            httpContext.Response.StatusCode = notFoundEx.HttpStatusCode;
            errorResponse = ApiResponseHelper.NotFound(message: notFoundEx.Message);
        }
        else if (exception is DatabaseDefinitionException databaseDefinitionException)
        {
            ...
            errorResponse = ApiResponseHelper.DatabaseDefinitionValueError(errors: databaseDefinitionException.ValidationErrors);
        }
        else if (exception is InternalServerException internalServerException)
        {
            ...
            errorResponse = ApiResponseHelper.InternalServerError(message: internalServerException.Message, errorDetail: errorDetail);
        }
        else if (exception is ExternalServiceException externalServiceException)
        {
            ...
            errorResponse = ApiResponseHelper.ExternalServiceError(message: externalServiceException.Message);
        }
        else if (exception is DatabaseExecuteException databaseExecuteException)
        {
            ...
            errorResponse = ApiResponseHelper.DatabaseExecuteError(message: databaseExecuteException.Message, errorDetail: errorDetail);
        }
        else if (exception is BusinessBadRequestException businessBadRequestException)
        {
            ...
            errorResponse = ApiResponseHelper.BusinessBadRequestError(message: businessBadRequestException.Message);
        }
        else if (exception is HeaderValidationException headerValidationException)
        {
            ...
            errorResponse = ApiResponseHelper.HeaderValidationError(message: headerValidationException.Message);
        }
        else
        {
            httpContext.Response.StatusCode = StatusCodes.Status500InternalServerError;
            string errorDetail = environment.IsDevelopment() ? exception.ToString() : "";
            errorResponse = ApiResponseHelper.InternalServerError(message: exception.Message, errorDetail: errorDetail);
        }
    ...
    )
    {
        ...
        var errorLog = new System_ErrorLog
        {
            ApplyNo = applyNo,
            Type = errorType,
            Project = SystemErrorLogProjectConst.INFOCHECKMIDDLEWARE,
            Source = path,
