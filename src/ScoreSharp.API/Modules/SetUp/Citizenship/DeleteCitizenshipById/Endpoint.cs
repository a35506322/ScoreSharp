using ScoreSharp.API.Modules.SetUp.Citizenship.DeleteCitizenshipById;

namespace ScoreSharp.API.Modules.SetUp.Citizenship
{
    public partial class CitizenshipController
    {
        /// <summary>
        /// 刪除單筆國籍
        /// </summary>
        /// <remarks>
        ///
        /// Sample Router:
        ///
        ///     /Citizenship/DeleteCitizenshipById/AE
        ///
        /// </remarks>
        /// <param name="code">國籍代碼</param>
        /// <returns></returns>
        [HttpDelete("{code}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(ResultResponse<string>))]
        [EndpointSpecificExample(
            typeof(刪除國籍_2000_ResEx),
            typeof(刪除國籍查無資料_4001_ResEx),
            ExampleType = ExampleType.Response,
            ResponseStatusCode = StatusCodes.Status200OK
        )]
        [OpenApiOperation("DeleteCitizenshipById")]
        public async Task<IResult> DeleteCitizenshipById([FromRoute] string code)
        {
            var result = await _mediator.Send(new Command(code));
            return Results.Ok(result);
        }
    }
}

namespace ScoreSharp.API.Modules.SetUp.Citizenship.DeleteCitizenshipById
{
    public record Command(string code) : IRequest<ResultResponse<string>>;

    public class Handler : IRequestHandler<Command, ResultResponse<string>>
    {
        private readonly ScoreSharpContext _context;

        public Handler(ScoreSharpContext context)
        {
            _context = context;
        }

        public async Task<ResultResponse<string>> Handle(Command request, CancellationToken cancellationToken)
        {
            var single = await _context.SetUp_Citizenship.SingleOrDefaultAsync(x => x.CitizenshipCode == request.code);

            if (single is null)
                return ApiResponseHelper.NotFound<string>(null, request.code);

            _context.SetUp_Citizenship.Remove(single);
            await _context.SaveChangesAsync();

            return ApiResponseHelper.DeleteByIdSuccess(request.code);
        }
    }
}
