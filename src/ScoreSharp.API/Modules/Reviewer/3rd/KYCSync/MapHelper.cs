...
{
    public static SyncKycMW3Info MapTo非卡友SyncKycMW3Info(
        bool 是否加上強押記號,
        Reviewer_ApplyCreditCardInfoMain main,
        Reviewer3rd_NameCheckLog? nameCheckLog
    )
    {
        SyncKycMW3Info syncKycMW3Info = new()
        {
            ...
            IsCurrentPositionRelatedPEPPosition = main.IsCurrentPositionRelatedPEPPosition is not null
                ...
                : string.Empty,
            NameCheckStartTime = nameCheckLog is not null ? nameCheckLog.StartTime.ToString("yyyy-MM-dd HH:mm:ss") : string.Empty,
            NameCheckResponseResult = nameCheckLog is not null
                ? nameCheckLog.ResponseResult == "Y"
                    ? KYCRequestConst.姓名檢核命中_是
                    : KYCRequestConst.姓名檢核命中_否
                : string.Empty,
            NameCheckRcPoint = nameCheckLog is not null ? nameCheckLog.RcPoint.ToString() : string.Empty,
            NameCheckAMLId = nameCheckLog is not null ? nameCheckLog.AMLId : string.Empty,
    ...
    public static SyncKycMW3Info MapTo卡友SyncKycMW3Info(Reviewer_ApplyCreditCardInfoMain main)
    {
        SyncKycMW3Info syncKycMW3Info = new()
        {
    ...
    )
    {
        ...
        var notes =
            $"KYC串接狀態: {isSuccessStr}; KYC Api: {apiCHName}; KYC 代碼: {kycCode}; KYC Level: {kycRank}; KYC Message: {kycMsg}; ({userTypeStr}_{id})";
        return new()
        {
            ApplyNo = applyNo,
            Process = process,
    ...
    public static QueryCustKYCRiskLevelRequestInfo MapTo紙本件原卡友QueryCustKYCRiskLevelRequestInfo(string id) =>
        new QueryCustKYCRiskLevelRequestInfo { ID = id };
}
