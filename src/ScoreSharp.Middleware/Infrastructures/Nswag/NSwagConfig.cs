namespace ScoreSharp.Middleware.Infrastructures.Nswag;
...
public static class NSwagConfig
{
    public static void AddNSwag(this IServiceCollection services, IHostEnvironment env)
    {
        ...
        services.AddOpenApiDocument(
            (settings, provider) =>
            {
                settings.Title = $"信用卡徵審系統 API Middleware ({env.EnvironmentName})";
                settings.Version = "v1";
                settings.Description = @"ScoreSharp Middleware API";
