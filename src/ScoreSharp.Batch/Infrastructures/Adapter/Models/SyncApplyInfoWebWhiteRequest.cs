...
public class SyncApplyInfoWebWhiteRequest : IValidatableObject
{
    ...
    /// </summary>
    [Display(Name = "正卡_本人同意提供資料予聯名(認同)集團")]
    [MaxLength(1)]
    ...
    /// 正卡_是否同意悠遊卡自動加值預設開啟
    /// </summary>
    [Display(Name = "正卡_是否同意悠遊卡自動加值預設開啟")]
    [MaxLength(1)]
    [RegularExpression(@"^[YN]$", ErrorMessage = "只能輸入Y或N")]
    ...
    {
        // 卡片狀態要在紙本件的狀態範圍內
        var allowedStatuses = new[] { CardStatus.網路件_書面申請等待列印申請書及回郵信封, CardStatus.網路件_書面申請等待MyData };
        if (CardInfo.Any(card => !allowedStatuses.Contains(card.CardStatus)))
            yield return new ValidationResult(
                $"卡片狀態不在{CardStatus.網路件_書面申請等待列印申請書及回郵信封}、{CardStatus.網路件_書面申請等待MyData}的範圍內，請檢查",
